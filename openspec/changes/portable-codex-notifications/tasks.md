...
- [x] 5.5 Add an idempotent Yazelix setup script that no-ops when `yzx` exists, installs the stable Nova branch through a Nix profile when Nix is available, defers successfully when Nix is missing, and never launches Yazelix; invoke it after notification setup and cover the observable behavior with isolated fixtures.
...
- [x] 5.10 Replace the legacy unqualified Yazelix reference with the canonical stable Nova channel, add one bounded retry for transient upstream profile build failures, preserve a persistent failure status, and cover both retry outcomes with isolated fixtures and documentation.
- [x] 5.11 Label the notification and Yazelix installer stages with their resolved paths so adjacent deferred-activation and Nix output cannot be mistaken as coming from the same script; cover both labels in bootstrap orchestration tests.
- [x] 5.12 Bound Nix's per-command download attempts during Yazelix setup, retain one outer retry for transient build failures, and report actionable GitHub HTTP 429 guidance after the second failure.
