...
Yazelix is not a Homebrew dependency. After the Brewfile and notification installer succeed, `macos.sh` runs `scripts/yazelix_setup.sh`. The script is idempotent: an existing `yzx` command is a no-op; otherwise, when Nix is available, it runs `nix profile add --refresh github:Yazelix/nova/stable`. If that profile command fails, setup reports the failure and retries the exact command once so a transient upstream build or check failure can recover. Each invocation sets `download-attempts` to one, bounding the overall operation at two attempts instead of allowing Nix's internal HTTP backoff to multiply the outer retry. A second failure remains fatal and visible, with specific guidance for GitHub HTTP 429 output. Nix remains missing only when installation was declined or unavailable before approval; that condition defers Yazelix and exits successfully so the completed Homebrew and notification setup is not retroactively treated as failed.

The bootstrap prints the resolved notification-installer and Yazelix-installer paths immediately before invoking each one. This keeps a notification installer's final deferred-activation message visually separate from subsequent Yazelix or Nix output.
...
- **An upstream Yazelix build check can fail transiently** → Install the dogfooded stable channel and retry the same profile command once; preserve the second failure so persistent package, network, or configuration errors remain visible.
- **GitHub anonymous archive downloads can return HTTP 429 with multi-minute backoff** → Limit each Nix command to one download attempt, keep the single outer retry, and direct the user to wait for the limit reset or configure Nix access tokens rather than silently looping.
