...
| `yazelix_input_source_context.py` | 唯讀狀態 helper。讀取 Yazelix Helix bridge 的真實 mode；優先使用 Ghostty tab 標題快速辨認 editor/sidebar，必要時才用 Zellij pane 查詢作為 fallback。 |
...
3. Helper 回傳 focused pane 與 Helix 的 `normal`、`select` 或 `insert` mode。
4. `helix_input_source.lua` 依規則呼叫 `input_source.lua` 切換或恢復輸入法。
